...
string_cohort<-paste("select nhs_number,dep_date as index_event_time
  from MODELLING_SQL_AREA.dbo.swd_activity
  where dep_date >= '2022-08-01'
  and arr_date <= '2022-08-01'
  and prov_code in ('ra700','rvj00')
  and pod_l1 = 'secondary'
  and pod_l2a = 'ip'
  and pod_l2b = 'non_elective'")
...
activity_coverage<-data.frame(activity_name=character(0),activity_filter=character(0)) %>%
  ...
  add_row(activity_name="999",activity_filter="pod_l1=='999'") %>%
...
report_title<-"What happens in 7 days after discharge for all NEL patients admitted on 1 August 2022"
